...
class NormalizedCycleDecomposer:
    ...
    def __init__(self, flow_df, valid_grids):
        ...
        # 災前原始震幅
        self.raw_amp_pre = np.maximum((max_dow - min_dow) / 2.0, 0.01)
...
# =========================================================================
# 5. 趨勢與震幅雙重指數擬合 + PCHIP 單調斜率連續重建引擎
# =========================================================================
def pchip_monotone_interpolate(y0, y1, d0, d1, tau):
    """
    PCHIP (Piecewise Cubic Hermite Interpolating Polynomial)
    嚴格保證單調性與斜率還原，徹底消滅駝峰 (No-Overshoot)
    """
    delta = y1 - y0
    d0_mod = np.copy(d0)
    d1_mod = np.copy(d1)
    
    # 符號與單調性約束 (Fritsch-Carlson 準則)
...
class TrendAmplitudeReconstructionEngine:
    def __init__(self, flow_df, decomposer, valid_grids, grid_class_lookup, is_offdiag=False):
        ...
        # 2. 建立 1/1 ~ 1/31 指數模型 (趨勢 mu_1 與 震幅 A_1)
        jan_df = flow_df.loc["2024-01-01":"2024-01-31", valid_grids]
        
        # 提取 1 月平滑趨勢 (移動 7 天中位數去週期)
        jan_smooth = jan_df.rolling(window=7, min_periods=1, center=True).median()
        self.mu_jan_start = robust_median(jan_smooth.head(4)).values
        self.mu_jan_end = robust_median(jan_smooth.tail(5)).values
        
        # 提取 1 月實際殘差震幅
        jan_res = (jan_df - jan_smooth).abs()
        self.A_jan_start = np.clip(robust_median(jan_res.head(7)).values * 1.4, 0.0, self.A_pre * 1.5)
        self.A_jan_end = np.clip(robust_median(jan_res.tail(7)).values * 1.4, 0.0, self.A_pre * 1.2)
        
        # 1 月指數衰減率 (Trend & Amplitude)
        self.k_mu_1 = np.full(len(valid_grids), 0.08)
        self.k_A_1 = np.full(len(valid_grids), 0.06)
        
        # 1 月底 (1/31, t=30) 出發斜率 (每日變化率)
        self.slope_mu_jan = -self.k_mu_1 * (self.mu_jan_start - self.mu_jan_end) * np.exp(-self.k_mu_1 * 30.0)
        self.slope_A_jan = -self.k_A_1 * (self.A_jan_start - self.A_jan_end) * np.exp(-self.k_A_1 * 30.0)
        
        # 3. 建立 4/1 ~ 10/31 指數模型 (趨勢 mu_2 與 震幅 A_2)
        ...
        self.mu_apr_start = robust_median(post_smooth.head(7)).values
        self.mu_long_term = robust_median(post_smooth.tail(30)).values
        
        post_res = (post_df - post_smooth).abs()
        self.A_apr_start = np.clip(robust_median(post_res.head(7)).values * 1.4, 0.0, self.A_pre * 1.2)
        self.A_long_term = np.clip(robust_median(post_res.tail(30)).values * 1.4, 0.0, self.A_pre)
        
        # 4~10 月指數漸近速率
        self.k_mu_2 = np.full(len(valid_grids), 0.02)
        self.k_A_2 = np.full(len(valid_grids), 0.015)
        
        # 4 月初 (4/1, t=0) 抵達斜率
        self.slope_mu_apr = self.k_mu_2 * (self.mu_long_term - self.mu_apr_start)
        self.slope_A_apr = self.k_A_2 * (self.A_long_term - self.A_apr_start)
    ...
    def get_backbone(self, dt: pd.Timestamp) -> np.ndarray:
        ...
        if dt < GAP_START:
            ...
            mu_t = self.mu_jan_end + (self.mu_jan_start - self.mu_jan_end) * np.exp(-self.k_mu_1 * t)
            A_t = self.A_jan_end + (self.A_jan_start - self.A_jan_end) * np.exp(-self.k_A_1 * t)
            
        elif dt <= GAP_END:
            # 2~3 月 Gap 期：PCHIP 單調連續插值重構
            t = (dt - GAP_START).days
            tau = t / float(gap_span - 1)
            
            # 將每日斜率乘以時間跨度 gap_span
            ...
            target_mu = self.mu_long_term.copy()
            target_A = self.A_long_term.copy()
            
            for i, g in enumerate(self.valid_grids):
                c = self.grid_class_lookup.get(g, 5)
                if c == 5:  # 完全復原區以災前基準為目標
                    target_mu[i] = self.M_pre[g]
                    target_A[i] = self.A_pre[i]
                    
            mu_t = self.mu_apr_start + (target_mu - self.mu_apr_start) * (1.0 - np.exp(-self.k_mu_2 * t))
            A_t = self.A_apr_start + (target_A - self.A_apr_start) * (1.0 - np.exp(-self.k_A_2 * t))

        # Off-diagonal 量級極小，震幅壓縮為微幅波動
        if self.is_offdiag:
            A_t = A_t * 0.15
...
morph_diag = TrendAmplitudeReconstructionEngine(diag_df, decomposer_diag, valid_grids, grid_class_lookup, is_offdiag=False)
morph_offdiag = TrendAmplitudeReconstructionEngine(offdiag_df, decomposer_offdiag, valid_grids, grid_class_lookup, is_offdiag=True)
...
class CorrectDDIMEngine:
    ...
    @torch.no_grad()
    def sample_ddim(self, model, norm_seed, time_feat, ddim_steps=DDIM_STEPS, clamp_val=1.2):
        b, dim = norm_seed.shape[0], model.num_nodes
        step_indices = torch.linspace(self.timesteps - 1, 0, ddim_steps).long().to(DEVICE)
        x = torch.randn((b, dim), device=DEVICE) * 0.4
...
# 嚴格控制殘差注入權重，維持精準的震幅與趨勢
CLASS_RES_WEIGHT_DIAG = {
    1: 0.00, 2: 0.08, 3: 0.12, 4: 0.08,
    5: 0.00, 6: 0.00, 7: 0.12, 8: 0.10, 9: 0.08
}
CLASS_RES_WEIGHT_OFFDIAG = {c: 0.0 for c in range(1, 10)}  # Offdiag 完全鎖定保護基準 0.0176
...
with torch.no_grad():
    for dt in all_dates:
        ...
        res_d = diff_engine.sample_ddim(diff_model_diag, n_seed_d, t_feat, ddim_steps=DDIM_STEPS, clamp_val=1.0).squeeze(0).cpu().numpy() * ds_diag.scale
        res_o = diff_engine.sample_ddim(diff_model_offdiag, n_seed_o, t_feat, ddim_steps=DDIM_STEPS, clamp_val=0.1).squeeze(0).cpu().numpy() * ds_offdiag.scale
        
        raw_d = base_d + alpha_d * res_d
        raw_o = base_o + alpha_o * res_o
...
print("-" * 110)
print(f"{'OVERALL (Trend+Amplitude Model)':<32} | {len(valid_grids):<5} | {RMSE_diag:10.4f} | {RMSE_offdiag:10.4f} | {NRMSE_diag:11.4f} | {NRMSE_offdiag:11.4f} | {combined_nrmse:14.4f}")
print("=" * 110 + "\n")
...
fig.suptitle(f"HuMob 2026: Slope-Continuous Trend + Adaptive Amplitude | Combined NRMSE: {combined_nrmse:.4f}", 
