...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "humob_shelter_diffusion_pure_output")
...
decomposer_diag = RobustCycleDecomposer(diag_df, valid_grids)
decomposer_offdiag = RobustCycleDecomposer(offdiag_df, valid_grids)

# =========================================================================
# 4. 全樣本聚合動態 OD 轉移引擎（不切分稀疏平日/週末）
# =========================================================================
class RobustShelterODEngine:
    ...
    def _build_transition_matrix(self, daily_od_records, target_dates):
        ...
        for orig in self.valid_grids:
            ...
            if tot > 0:
                # 僅保留有實質流動的路徑，且直接依原始比例歸一化
                probs[orig] = {d: c / tot for d, c in counts[orig].items() if (c / tot) >= 0.005}
                sub_tot = sum(probs[orig].values())
                probs[orig] = {d: p / sub_tot for d, p in probs[orig].items()} if sub_tot > 0 else {}
            else:
                probs[orig] = {}
        return probs
...
od_engine = RobustShelterODEngine(valid_grids, grid_class_lookup, daily_od_records, dist_matrix)
...
# =========================================================================
# 6. 專注 Diag 訓練的輕量 Diffusion (Off-Diag 維持零殘差純淨模式)
# =========================================================================
...
class ResidualDataset(Dataset):
    def __init__(self, flow_df, morph_engine, valid_grids):
        self.samples = []
        ...
        self.scale = np.where(self.scale < 0.5, 0.5, self.scale)
...
class StandardDiffusionEngine:
    ...
    @torch.no_grad()
    def sample(self, model, norm_seed, time_feat, clamp_val=1.5):
        b, dim = norm_seed.shape[0], model.num_nodes
        x_t = torch.randn(b, dim, device=DEVICE)
        
        for step in reversed(range(self.timesteps)):
            t_t = torch.full((b,), step, device=DEVICE, dtype=torch.long)
            betas_t = self.betas[t_t].unsqueeze(-1)
            pred_eps = model(x_t, t_t, norm_seed, time_feat)
            mean = (1.0 / torch.sqrt(self.alphas[t_t].unsqueeze(-1))) * (x_t - (betas_t / self.sqrt_one_minus_alphas_cumprod[t_t].unsqueeze(-1)) * pred_eps)
            if step > 0:
                x_t = mean + torch.sqrt(self.posterior_var[t_t].unsqueeze(-1)) * torch.randn_like(x_t)
            else:
                x_t = mean
                
        return torch.clamp(x_t, -clamp_val, clamp_val)

print("\n[2/6] 準備對角線殘差並訓練 Diag Diffusion 模型...")
ds_diag = ResidualDataset(diag_df, morph_diag, valid_grids)
loader_diag = DataLoader(ds_diag, batch_size=BATCH_SIZE, shuffle=True)

diff_engine = StandardDiffusionEngine(DIFFUSION_STEPS)
diff_model_diag = WaveformDenoiser(num_nodes=num_nodes, hidden_dim=96).to(DEVICE)

def train_diffusion(model, loader):
    ...
    for ep in range(1, EPOCHS_DIFFUSION + 1):
        ...
        if ep % 20 == 0 or ep == EPOCHS_DIFFUSION:
            print(f"  [Diag Diffusion] Epoch [{ep:02d}/{EPOCHS_DIFFUSION}] - Loss: {total_loss / len(loader):.5f}")

train_diffusion(diff_model_diag, loader_diag)

# =========================================================================
# 7. 全時段融合推論
# =========================================================================
print("\n[3/6] 執行全時段 (1~10月) 高純度推論...")
...
with torch.no_grad():
    for dt in all_dates:
        ...
        # 2. 僅對 Diag 進行微觀殘差採樣 (Off-Diag 嚴格保持純淨骨架以避免 0.0176 爆炸)
        ...
        res_d = diff_engine.sample(diff_model_diag, n_seed_d, t_feat, clamp_val=1.5).squeeze(0).cpu().numpy() * ds_diag.scale
        
        raw_d = base_d + 0.25 * res_d
        raw_o = base_o  # Off-diagonal 100% 保持乾淨物理骨架
        
        # 3. 空間一致性微平滑
        smooth_d = 0.98 * raw_d + 0.02 * spatial_knn.dot(raw_d).values
        smooth_o = 0.995 * raw_o + 0.005 * spatial_knn.dot(raw_o).values
        
        final_d = np.clip(smooth_d, 0.0, ds_diag.max_ceiling)
        final_o = np.clip(smooth_o, 0.0, morph_offdiag.max_ceiling.values)
...
print("\n" + "=" * 110)
print(" 🏆 【HuMob 2026 純淨穩健版評估報告】(去噪聚焦 + 聚合轉移池)")
...
print("-" * 110)
print(f"{'OVERALL (Pure Robust Model)':<32} | {len(valid_grids):<5} | {RMSE_diag:10.4f} | {RMSE_offdiag:10.4f} | {NRMSE_diag:11.4f} | {NRMSE_offdiag:11.4f} | {combined_nrmse:14.4f}")
print("=" * 110 + "\n")

df_metrics.to_csv(os.path.join(OUTPUT_DIR, "pure_nrmse_breakdown.csv"), index=False, encoding="utf-8-sig")
...
pred_total_df.to_csv(os.path.join(OUTPUT_DIR, "pred_total_flows_pure.csv"), encoding="utf-8-sig")
...
fig.suptitle(f"HuMob 2026: Pure Robust Diffusion Pipeline | Combined NRMSE: {combined_nrmse:.4f}", 
...
for c_id in range(1, 10):
    ...
    ax.plot(pred_series.index, pred_series, color='#38bdf8', linewidth=1.3, label='Pure Model' if c_id == 1 else "")
...
plt.savefig(os.path.join(OUTPUT_DIR, "pure_9classes_comparison.png"), dpi=250, bbox_inches='tight')
plt.close(fig)

print(f"[6/6] ✨ 純淨穩健版執行完畢！所有結果已儲存至：{OUTPUT_DIR}")
