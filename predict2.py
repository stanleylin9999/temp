...
print("=" * 80)
print("🚀 HuMob 動態非線性預測流程：類別形態學 ➔ AR(1)微波動 ➔ 防暴衝截斷 ➔ NRMSE 評估")
...
# 5. 執行全期動態生成 (含受控微波動 AR(1) 與類別動態 Envelope)
np.random.seed(42)
...
for dt in full_pred_dates:
    ...
    if dt < GAP_START:
        ...
        for g in valid_grids:
            ...
            if c in [3, 7]:  # 應急活動/臨時激增：在 2~3 月形成救援活動波峰
                peak_factor = np.sin(np.pi * tau) * 0.45 * l_jan_end[g]
                mu_t[g] += peak_factor
            elif c == 4:     # 部分復原：早期緩慢、中後期抬升
...
fig_grid.suptitle(f'Dynamic 9-Class Predictions (Anti-Over-Smoothing) | Combined NRMSE: {final_score:.4f}', 
