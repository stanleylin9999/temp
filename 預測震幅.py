...
FALLBACK_CLASS_DIR = os.path.join(
    SCRIPT_DIR,
    "humob2026",
    "data",
    "output",
...
BY_CLASS_DIR = (
    SPECIFIC_CLASS_DIR if os.path.exists(SPECIFIC_CLASS_DIR) else FALLBACK_CLASS_DIR
)
    ...
    if c_id is not None:
      try:
        ...
        valid_cols = [
            ...
            if any(k in str(c).lower() for k in ["grid", "orig"])
        ]
        if not valid_cols:
          valid_cols = [
              c
              for c in df_cls.columns
              if "id" in str(c).lower() and "class" not in str(c).lower()
          ]
        col = valid_cols[0] if valid_cols else df_cls.columns[0]
  ...
  try:
    ...
    for orig, dests in od_dict.items():
      ...
      if 30 <= x_idx <= 70 and 35 <= y_idx <= 70:
        ...
        daily_offdiag[dt][orig] = sum(
            float(cnt)
            for dest, cnt in dests.items()
...
if not valid_grids:
  valid_grids = diag_df.columns[
      diag_df[diag_df.index < PRED_START].mean() >= 0.001
  ].tolist()
...
grid_classes = np.array(
    [grid_class_lookup.get(g, 5) - 1 for g in valid_grids], dtype=np.int64
)
...
all_channels_3d = np.stack(
    [obs_mu_d.values, obs_amp_d.values, obs_mu_o.values, obs_amp_o.values],
    axis=-1,
).astype(np.float32)

base_feats_2d = np.stack(
    [base_mu_d.values, base_amp_d.values, base_mu_o.values, base_amp_o.values],
    axis=-1,
).astype(np.float32)

# =========================================================================
# 4. 向量化三次埃爾米特基底與保單調插值器 (PCHIP)
# =========================================================================
tau = np.linspace(0.0, 1.0, BRIDGE_LEN, dtype=np.float32)  # [62]
h00 = (2.0 * (tau**3) - 3.0 * (tau**2) + 1.0).reshape(1, 1, BRIDGE_LEN)
h10 = ((tau**3) - 2.0 * (tau**2) + tau).reshape(1, 1, BRIDGE_LEN)
h01 = (-2.0 * (tau**3) + 3.0 * (tau**2)).reshape(1, 1, BRIDGE_LEN)
h11 = ((tau**3) - (tau**2)).reshape(1, 1, BRIDGE_LEN)
...
def build_pchip_bridge(v_l, s_l, v_r, s_r, span_len=BRIDGE_LEN):
  delta = v_r - v_l
  d_l = s_l * span_len
  d_r = s_r * span_len

  sign_delta = np.sign(delta)
  d_l_clamped = np.where(sign_delta * d_l > 0, d_l, 0.0)
  d_r_clamped = np.where(sign_delta * d_r > 0, d_r, 0.0)

  max_slope = 2.0 * np.abs(delta)
  d_l_clamped = np.clip(d_l_clamped, -max_slope, max_slope)
  d_r_clamped = np.clip(d_r_clamped, -max_slope, max_slope)

  d_l_clamped = np.where(delta == 0, 0.0, d_l_clamped)
  d_r_clamped = np.where(delta == 0, 0.0, d_r_clamped)

  v_l_exp = v_l[..., None]
  v_r_exp = v_r[..., None]
  d_l_exp = d_l_clamped[..., None]
  d_r_exp = d_r_clamped[..., None]

  return h00 * v_l_exp + h10 * d_l_exp + h01 * v_r_exp + h11 * d_r_exp
...
target_windows = np.array(
    [post_data[s : s + BRIDGE_LEN] for s in start_indices]
)
...
base_hermite_train = build_hermite_base(
    v_left, slope_left, v_right, slope_right, span_len=BRIDGE_LEN
)
cond_features = np.concatenate(
    [v_base, v_left, slope_left, v_right, slope_right], axis=-1
)
...
train_loader = DataLoader(
    TensorDataset(X_train, C_train, B_train, Y_train),
...
print(
    f"[3/5] 極速訓練硬邊界約束網路 (樣本數: {len(X_train)}, Epochs:"
    f" {EPOCHS})..."
)
...
for ep in range(1, EPOCHS + 1):
  ...
  if ep % 5 == 0 or ep == EPOCHS:
    print(
        f"  Epoch [{ep:02d}/{EPOCHS:02d}] - Loss:"
...
s_l_real = (
    all_channels_3d[idx_jan_end] - all_channels_3d[idx_jan_end - 3]
) / 3.0
v_r_real = all_channels_3d[idx_apr_start]
s_r_real = (
    all_channels_3d[idx_apr_start + 3] - all_channels_3d[idx_apr_start]
) / 3.0
v_b_real = base_feats_2d

test_feat = np.concatenate(
    [v_b_real, v_l_real, s_l_real, v_r_real, s_r_real], axis=-1
)

base_hermite_test = build_hermite_base(
    v_l_real, s_l_real, v_r_real, s_r_real, span_len=BRIDGE_LEN
)
...
with torch.no_grad():
  ...
  pred_pinned = (
      model(t_x, t_c, t_b).view(num_nodes, 4, BRIDGE_LEN).cpu().numpy()
  )

# -------------------------------------------------------------------------
# 針對特定類別修正：利用保單調 PCHIP 徹底消除 C2/C3/C7 的巨幅氣球凸起、駝峰與深坑
# -------------------------------------------------------------------------
pchip_bridge_all = build_pchip_bridge(
    v_l_real, s_l_real, v_r_real, s_r_real, span_len=BRIDGE_LEN
)

# 【Class 1: Persistent Zero】維持全零
is_c1 = grid_classes == 0
pred_pinned[is_c1] = 0.0

# 【Class 2: Persistent Decrease】
# 替換為保單調過渡：Stay Flow 保持在 2.0~2.3 平緩低迷，Cross Flow 保持在 0.02~0.04 不墜地
is_c2 = grid_classes == 1
if np.any(is_c2):
  pred_pinned[is_c2] = pchip_bridge_all[is_c2]

# 【Class 3: Emergent Activity】
# 替換為保單調過渡：Stay Flow 穩定在 0.07~0.08 的應急平台，徹底消除 0.30 巨大拱頂
is_c3 = grid_classes == 2
if np.any(is_c3):
  pred_pinned[is_c3] = pchip_bridge_all[is_c3]

# 【Class 7: Temporary Increase】
# 消除 2~3 月不合理的向上隆起二次駝峰，強制採用 PCHIP 保單調衰退平滑回穩
is_c7 = grid_classes == 6
if np.any(is_c7):
  pred_pinned[is_c7] = pchip_bridge_all[is_c7]

pred_pinned = np.maximum(pred_pinned, 0.0)

bridge_dates = pd.date_range(BRIDGE_START, BRIDGE_END, freq="D")

pinned_mu_diag = pd.DataFrame(
    pred_pinned[:, 0, :].T, index=bridge_dates, columns=valid_grids
)
pinned_amp_diag = pd.DataFrame(
    pred_pinned[:, 1, :].T, index=bridge_dates, columns=valid_grids
)
pinned_mu_offdiag = pd.DataFrame(
    pred_pinned[:, 2, :].T, index=bridge_dates, columns=valid_grids
)
pinned_amp_offdiag = pd.DataFrame(
    pred_pinned[:, 3, :].T, index=bridge_dates, columns=valid_grids
)
...
gap_mu_diag.to_csv(
    os.path.join(OUTPUT_DIR, "pinned_gap_mu_diagonal.csv"),
    encoding="utf-8-sig",
)
gap_amp_diag.to_csv(
    os.path.join(OUTPUT_DIR, "pinned_gap_amp_diagonal.csv"),
    encoding="utf-8-sig",
)
gap_mu_offdiag.to_csv(
    os.path.join(OUTPUT_DIR, "pinned_gap_mu_offdiagonal.csv"),
    encoding="utf-8-sig",
)
gap_amp_offdiag.to_csv(
    os.path.join(OUTPUT_DIR, "pinned_gap_amp_offdiagonal.csv"),
    encoding="utf-8-sig",
)

# =========================================================================
# 8. 繪製面板圖 (預測虛線為綠色 #10b981)
# =========================================================================
print("[5/5] 繪製首尾無縫對齊面板圖 (預測虛線為綠色)...")


def plot_pinned_panel(
    obs_mu, obs_amp, pin_mu, pin_amp, base_mu, base_amp, tag_name, save_name
):
  plt.style.use("dark_background")
  fig, axes = plt.subplots(9, 2, figsize=(18, 22), dpi=220, sharex=True)
  fig.patch.set_facecolor("#090d16")
  fig.suptitle(
      f"HuMob 2026: Boundary-Pinned Trend μ(t) & Amplitude A(t) [{tag_name}]\n(Green"
      " Dashed Bridge = Predicted Infill Seamlessly Connected)",
      fontsize=14.5,
      fontweight="bold",
      color="#ffffff",
      y=0.99,
  ...
  for c_id in range(1, 10):
    ...
    for ax in (ax_mu, ax_amp):
      ...
      ax.axvspan(
          ...
          color="#475569",
          alpha=0.35,
          label="Gap Period" if c_id == 1 else "",
      )
      ax.axvline(
          PRED_START,
          color="#ef4444",
          ...
          linewidth=1.1,
          alpha=0.85,
          label="Earthquake" if c_id == 1 else "",
      )
    ...
    ax_mu.plot(
        ...
        color="#38bdf8",
        linewidth=1.3,
        label="Observed μ(t)" if c_id == 1 else "",
    )
    ax_mu.plot(
        mu_pin_c.index,
        mu_pin_c,
        ...
        linestyle="--",
        linewidth=1.9,
        label="Predicted (Gap Infill)" if c_id == 1 else "",
    )
    ax_mu.axhline(
        b_mu,
        color="#eab308",
        ...
        linewidth=1.0,
        alpha=0.75,
        label="Pre Baseline" if c_id == 1 else "",
    )
    ...
    ax_amp.plot(
        ...
        color="#f43f5e",
        linewidth=1.3,
        label="Observed A(t)" if c_id == 1 else "",
    )
    ax_amp.plot(
        amp_pin_c.index,
        amp_pin_c,
    ...
    if c_id == 1:
      ax_mu.set_title(
          "Trend Mean μ(t) [Low-Frequency Base]",
          fontsize=11,
          fontweight="bold",
          color="#38bdf8",
          pad=6,
      ...
      ax_amp.set_title(
          "Envelope Amplitude A(t) [Weekly Fluctuation]",
          fontsize=11,
          fontweight="bold",
          color="#f43f5e",
          pad=6,
  ...
  for col in (0, 1):
    ...
    ax_bottom.set_xlim(
        pd.to_datetime("2023-10-15"), pd.to_datetime("2024-11-15")
    )
...
err_jan = np.max(
    np.abs(
        obs_mu_d.loc["2024-01-31"].values - pinned_mu_diag.loc["2024-01-31"].values
    )
)
err_apr = np.max(
    np.abs(
        obs_mu_d.loc["2024-04-01"].values - pinned_mu_diag.loc["2024-04-01"].values
    )
)
print(f"  * 1 月 31 日 (左邊界端點) 最大誤差: {err_jan:.10e} (完全對齊)")
print(f"  * 4 月 01 日 (右邊界端點) 最大誤差: {err_apr:.10e} (完全對齊)")
print("=" * 90)
print(f"\n✨ 執行完畢！所有修正後成果均已儲存至：{OUTPUT_DIR}")
